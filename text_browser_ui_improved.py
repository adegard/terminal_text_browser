...
# ========= NEW UI HELPER FUNCTIONS (v1.54) =========

def draw_box(title="", width=None):
    """Draw a styled box header"""
    if width is None:
    ...
    if title:
        ...
        print(f"{C_TITLE}│ {title_str.center(box_width - 4)} │{C_RESET}")
        print(f"{C_DIM}├{'─' * (box_width - 2)}┤{C_RESET}")
...
def progress_bar_enhanced(current, total, width=25):
...
def show_ai_answer(answer):
    ...
    while True:
        ...
        print()
        print(f"{C_DIM}Paragraph {page+1}/{len(paragraphs)}{C_RESET}")
        print(f"{C_CMD}↑/p=prev  ↓/ENTER=next  q=quit{C_RESET}")
        print(f"{C_DIM}{'─' * shutil.get_terminal_size().columns}{C_RESET}\n")
...
def auto_update():
    clear_screen()
    draw_box("AUTO UPDATE")
    print(f"Current version: {APP_VERSION}\n")

    remote = get_remote_version()
    if not remote:
        show_message("error", "Update Check Failed", "Could not fetch remote version")
        input("\nPress ENTER…")
        return
    ...
    if remote == APP_VERSION:
        show_message("success", "Up to Date", "You are already using the latest version")
        input("\nPress ENTER…")
        return

    show_message("warning", "New Version Available", f"Latest: {remote}")
    print("Downloading…")

    new_script = download_latest_script()
    if not new_script:
        show_message("error", "Download Failed", "Could not download new script")
        input("\nPress ENTER…")
        return
    ...
    try:
        ...
        show_message("success", "Update Complete", f"Backup saved: {backup_file}")
        ...
        show_message("error", "Update Failed", str(e))
        input("\nPress ENTER…")
...
def extract_pdf_text(url):
    ...
    if pdf_bytes is None:
        try:
            ...
            return [f"[PDF fetch error: {e}]"], "Error"
    ...
    try:
        ...
        return [f"[PDF parse error: {e}]"], "Error"
    ...
    if not paragraphs:
        return ["[PDF contains no extractable text]"], "Empty"
...
def print_search_results_page(results_page, page_idx, total_pages):
    ...
    for i, (title, url) in enumerate(results_page, 1):
        ...
        print(f"{C_LINK}{i:2d}. {title[:cols-10]}{C_RESET}")
        short_url = shorten_middle(url, cols - 8)
        print(f"    {C_DIM}→ {short_url}{C_RESET}")
    
    print()
    print(f"{C_DIM}Page {page_idx+1}/{total_pages}{C_RESET}")
    print(f"{C_CMD}[1-{len(results_page)}]=open  [ENTER]=next  p=prev  bm=bookmarks  h=home  q=quit{C_RESET}")
...
def settings_menu():
    ...
    while True:
        ...
        print(f"🤖 5. Set Groq API key (current: {'✓ SET' if GROQ_API_KEY else '✗ NOT SET'})")
        ...
        print(f"✓ 9. Show page title: {SHOW_PAGE_TITLE}")
        ...
        if c == "1":
            ...
            if val.isdigit() and 1 <= int(val) <= 20:
                ...
                save_config()
                show_message("success", "Updated", f"Paragraphs per page: {PARAS_PER_PAGE}")
            continue

        if c == "2":
            clear_screen()
            draw_box("SEARCH ENGINES")
            print()
            ...
            if s.isdigit():
                ...
                if 0 <= idx < len(keys):
                    ...
                    save_config()
                    show_message("success", "Updated", f"Search engine: {SEARCH_ENGINES[DEFAULT_ENGINE]}")
            continue
        ...
        if c == "3":
            ...
            if val.isdigit() and 5 <= int(val) <= 50:
                ...
                save_config()
                show_message("success", "Updated", f"Results per page: {SEARCH_RESULTS_PER_PAGE}")
            continue
        ...
        if c == "4":
            ...
            apply_color_theme()
            save_config()
            show_message("success", "Updated", f"Color theme: {COLOR_THEME}")
        ...
        if c == "5":
            ...
            if key:
                GROQ_API_KEY = key
                save_config()
                show_message("success", "Saved", "Groq API key has been saved")
            continue
        if c == "6": 
            ...
            if val.isdigit() and 1 <= int(val) <= 200: 
                ...
                save_config()
                show_message("success", "Updated", f"Chronology length: {CHRONOLOGY_LENGTH}")
            continue
        ...
        if c == "7":
            ...
            if val.isdigit() and 200 <= int(val) <= 5000:
                ...
                save_config()
                show_message("success", "Updated", f"Max chars per block: {MAX_CHARS_PER_BLOCK}")
            continue

        if c == "8":
            SHOW_READING_MENUS = not SHOW_READING_MENUS
            save_config()
            show_message("info", "Toggled", f"Show menus: {SHOW_READING_MENUS}")
            continue

        if c == "9":
            SHOW_PAGE_TITLE = not SHOW_PAGE_TITLE
            save_config()
            show_message("info", "Toggled", f"Show page title: {SHOW_PAGE_TITLE}")
            continue

        if c == "10":
            SHOW_PROGESS_BAR = not SHOW_PROGESS_BAR
            save_config()
            show_message("info", "Toggled", f"Show progress bar: {SHOW_PROGESS_BAR}")
            continue
        ...
        if c == "11":
            ...
            if val.isdigit() and 60 <= int(val) <= 300:
                ...
                save_config()
                show_message("success", "Updated", f"PDF WPM: {ADAPTIVE_WPM_PDF}")
            continue
        ...
        if c == "12":
            ...
            if val.isdigit() and 60 <= int(val) <= 300:
                ...
                save_config()
                show_message("success", "Updated", f"HTML WPM: {ADAPTIVE_WPM_HTML}")
            continue
    ...
    |    |  /   \ |   |      |
     \   \  \___/ /   /     /
      '.  '._   _.'  .'     /
        ...
        print(f"{C_DIM}(Search text / ifl+text / Url / bm / c / s / ai+text / q){C_RESET}\n")
        ...
        if low.startswith("ifl "):
            q = t[4:].strip()
            if not q:
                show_message("warning", "Usage", "ifl <search terms>")
                continue

            results = search(q)
            if not results:
                show_message("error", "No Results", "Try a different search")
                input("Enter…")
                continue
        ...
        if t.startswith("ai "):
            question = t[3:].strip()
            if not question:
                show_message("info", "Usage", "ai your question")
                continue

            if not GROQ_API_KEY:
                show_message("error", "No API Key", "Go to Settings → Set Groq API key")
                continue
...
def search_and_select(query):
    ...
    if not results:
        show_message("error", "No Results", "Try a different search query")
        input("Enter…")
        return None
    ...
    while True:
        ...
        show_message("error", "Invalid Input", "Please enter a valid option")
...
def bookmark_manager():
    while True:
        ...
        if not b:
            show_message("info", "Empty", "No bookmarks saved yet")
            input("\nEnter…")
            return None
        ...
        for i, (title, url, block) in enumerate(b, 1):
            ...
            print(f"{C_LINK}{i}. {label}{C_RESET}")
            print(f"   {C_DIM}{short_url}  [block {block}]{C_RESET}")

        print(f"\n{C_CMD}[1-{len(b)}]=open  d#=delete  q=back{C_RESET}")
        ...
        if c.startswith("d") and c[1:].isdigit():
            ...
            show_message("success", "Deleted", "Bookmark removed")
            continue
        if c.isdigit():
            i = int(c) - 1
            if 0 <= i < len(b):
...
def chronology_manager():
    while True:
        clear_screen()
        h = load_history()
        draw_box("CHRONOLOGY (HISTORY)")
        print()

        if not h:
            show_message("info", "Empty", "No history yet")
            input("\nEnter…")
            return None
        ...
        print(f"\n{C_CMD}[1-{len(h)}]=open  q=back{C_RESET}")
...
def show_page(url, origin, start_block=0):
    ...
    try:
        ...
        show_message("error", "Fetch Failed", str(e))
        input("Enter…")
        return ("home",)
    # <<< END FIX
    ...
    while True:
        ...
        if mode == "text":
            ...
            pb_enhanced = progress_bar_enhanced(page + 1, len(text_pages), width=25)
                                           
            if SHOW_PROGESS_BAR:
                # --- Progress Bar & Remaining reading time ---
                if is_pdf:
                ...
                print(f"{pb_enhanced} {C_DIM}({remaining} remaining){C_RESET}")
            ...
            if SHOW_READING_MENUS:
                print(f"\n{C_CMD}Space/↓=next  p/↑=prev  l=links  i=image  b=back  m=save  s=share  h=home  q=quit{C_RESET}")
        ...
        # ---------------- LINKS MODE ----------------
            ...
            if link_pages and 0 <= page < len(link_pages):
                ...
                for i, (label, link) in enumerate(link_pages[page], 1):
                    ...
                    print(f"{C_LINK}{i}. {short_label}{C_RESET}")
                    print(f"   {C_DIM}→ {short_link}{C_RESET}")
                print()
                print(f"{C_DIM}Page {page+1}/{len(link_pages)}{C_RESET}")
            else:
                print("[No links]\n")

            print(
                f"{C_CMD}[SPACE]=next  p=prev  [1-5]=open  t=text  q=quit{C_RESET}"
            )

        # ---------------- TITLE & BOOKMARKED----------------
        # Bookmark indicator
        if is_bookmarked(url):
            bm_flag = f"{C_CMD}✓{C_RESET}"
        else:
            bm_flag = ""
        ...
        if c == "bc":
            hist = load_history()
            if len(hist) < 2:
                show_message("info", "No History", "No previous page in chronology")
                continue
        ...
        if c == "m":
            ...
            show_message("success", "Saved", "Bookmark saved successfully")
            continue
        if c == "bm":
            bm = bookmark_manager()
            if bm:
        ...
        if c == "s":
            ...
            print(f"{C_DIM}Press ENTER to continue{C_RESET}")
            input()
            continue
        ...
        if mode == "text":
            ...
            if c == "next":
                ...
                show_message("info", "End of Content", "No more content to read")
                continue
            ...
            if c == "i":
                if not main_image:
                    show_message("error", "No Image", "No image found in this article")
                    continue

                clear_screen()
                draw_box("ARTICLE IMAGE")
                print()
        ...
        show_message("error", "Invalid Input", "Please enter a valid command")
